...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	meanSpeed := meanSpeed(action, duration) // ваш код здесь
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	meanSpeesMs := meanSpeed(action, duration) * kmhInMsec
	heightInMeters := height / 100 // ваш код здесь
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeesMs, 2)/heightInMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
